...
import translate from "../../user/assets/translationFiles/addUserWizardTranslation";
import {
  useGetPatientPhoto,
...
const UploadProfilePicture: React.FC<Props> = (props) => {
  ...
  const { mutate: uploadPhoto } = useUploadPatientPhoto();
  const { data: photoURL } = useGetPatientPhoto(parseInt(props.patientId, 10));
        ...
        success: (result) => {
          ...
          if (image.src.indexOf("blob") > -1) {
            blobToBase64(event.target.files[0]).then((res: any) => {
              uploadPhoto({ patId: props?.patientId, imageBase64: res });
              setImageState(res);
            });
          } else {
            uploadPhoto({ patId: props?.patientId, imageBase64: image.src });
  ...
  const validateImage = () => {
    if (!(file === ""))
      uploadPhoto({ patId: props?.patientId, imageBase64: imageState });
  };
