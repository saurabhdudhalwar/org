...
const CellmaDistrictSearch: React.FC<Props> = (props) => {
  ...
  const { districts } = useDistrictSearch(
    districtQuery.current,
    districtQuery.current !== ""
  );

  const handleDistrict = (newValue: any, data: FormikProps<any>) => {
  ...
  const handleInput = (data: FormikProps<any>) => {
    ...
    <Autocomplete
      ...
      renderInput={(params) => (
        <TextField
          ...
          value={district}
          name="districtSearch"
          label={translate("districtSearch", language)}
          onBlur={() => {
            setDistrict("");
          }}
          inputProps={{
